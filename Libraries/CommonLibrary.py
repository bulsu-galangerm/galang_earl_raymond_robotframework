...
class CommonLibrary():
    def get_users(self):
        ...
        for user in users:
            ...
            user["address"]["stateAbbr"] = str(user["address"]["street"][0])+str(user["address"]["city"][0])
    ...
    def get_random_birthday(self):
        return f"{str(random.randint(1,12)).zfill(2)}/{str(random.randint(1,2)).zfill(2)}/{str(random.randint(1999,2006))}"
